...
Per-user dotfiles, theming, and day-to-day management. Everything is self-contained to an individual user — never requires sudo for day-to-day operations.
...
SL_KB_LAYOUT="pl" # keyboard layout, e.g. "pl", "us", "de"
...
The repo includes:
- `.editorconfig` — enforces consistent editor settings (2-space indent, LF line endings, UTF-8)
- `.shellcheckrc` — ShellCheck configuration (disables SC1090/SC1091 for sourced files)
- `.github/workflows/shellcheck.yml` — GitHub Actions CI that runs ShellCheck on push/PR for all `.sh`, `.lib.sh`, `.bashrc`, and `.sl-*` files
